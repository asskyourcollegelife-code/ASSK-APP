...
export default async function AdminEventsPage() {
    ...
    const getCategoryBadge = (category: string) => {
        switch (category?.toLowerCase()) {
            ...
            case 'workshop': return 'bg-purple-50 text-purple-600 border-purple-100'
        ...
        <div className="max-w-7xl mx-auto pb-12">
            ...
            <div className="bg-white rounded-3xl p-5 lg:p-8 shadow-sm border border-gray-100 mb-6 lg:mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 lg:gap-6 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 relative z-10">
                    <div className="bg-pink-100 text-pink-600 p-4 rounded-2xl shrink-0 hidden md:block">
                        <Calendar size={32} strokeWidth={2.5} />
                    </div>
                    <div>
                        <h1 className="text-2xl lg:text-3xl font-extrabold text-gray-900 mb-1">Campus Events</h1>
                        <p className="text-gray-500 font-medium text-sm lg:text-base">Manage upcoming workshops, fests, and club activities.</p>
                    </div>
                </div>

                <div className="flex relative z-10 w-full md:w-auto mt-2 md:mt-0">
                    <Link href="/admin/events/new" className="w-full justify-center bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold transition-colors shadow-sm shadow-indigo-200 flex items-center gap-2">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50 border-b border-gray-100">
                                <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider w-2/5">Event Details</th>
                                ...
                                <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">RSVPs</th>
                                    ...
                                    <tr key={event.id} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="p-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${getCategoryBadge(event.category)}`}>
                                                    {event.category}
                                                </span>
                                                {event.club && (
                                        ...
                                        <td className="p-4">
                                            <p className="text-sm font-bold text-gray-700">{new Date(event.event_date).toLocaleDateString()}</p>
                                            <p className="text-xs font-medium text-gray-500">{event.start_time?.substring(0, 5) || 'TBA'}</p>
                                        </td>
                                        <td className="p-4 text-sm font-medium text-gray-600 truncate max-w-[150px]">
                                            {event.venue || 'TBA'}
                                        </td>
                                        <td className="p-4">
